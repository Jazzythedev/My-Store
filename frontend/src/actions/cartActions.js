...
localStorage.setItem('cartItems', JSON.stringify(getState().cart.cartItems))                                    /* THIS IS HOW YOU ADD INFO INTO YOUR LOCAL STORAGE. keeps data alive as longs as browser is still open.  include key that was made in store.js "cartItems'. call the getState function to go to redux and grab existing cart for you. .cart is the friendly name for the cartreducer. cartItems is needed also bec it has the latest item that was added, we want all of it. convert all this to a string as local storage stores in strings.  use the json lib which has a stringhfy that converts a json object into a string. this will now be stored in local storage.*/
